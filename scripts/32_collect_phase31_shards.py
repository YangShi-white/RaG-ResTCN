...
def main() -> None:
    ...
    per_batch_files = sorted(metrics_dir.glob("per_batch_metrics_shard*.csv"))
    ...
    per_batch_rows = [row for path in per_batch_files for row in read_rows(path)] if per_batch_files else []
    run_rows = [row for path in run_files for row in read_rows(path)]
    write_rows(metrics_dir / "per_target_metrics.csv", metric_rows)
    if per_batch_rows:
        write_rows(metrics_dir / "per_batch_metrics.csv", per_batch_rows)
    ...
    if per_batch_rows:
        print(f"Per-batch rows: {len(per_batch_rows)}")
