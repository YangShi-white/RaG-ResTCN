...
def build_model(model_name: str, params: dict[str, Any], history_steps: int, n_hist: int, n_future: int, n_targets: int, nn: Any, torch: Any) -> Any:
    ...
    if model_name in {"linear_mlp", "dlinear"}:
        return LinearMLPBaseline(history_steps, n_hist, n_future, n_targets, nn, **params)
    if model_name == "dlinear_control":
        return DLinearControl(history_steps, n_hist, n_future, n_targets, nn, **params)
    ...
    def __new__(
        ...
        class _Model(nn.Module):
            def __init__(self) -> None:
                ...
                k = int(kernel_size)
                if k < 1 or k % 2 == 0:
                    raise ValueError("DLinear-Control kernel_size must be a positive odd integer")
                self.kernel_size = k
                self.individual = bool(individual)
                self.avg_pool = nn.AvgPool1d(kernel_size=k, stride=1, padding=k // 2, count_include_pad=False)
                if self.individual:
                    self.trend_weight = nn.Parameter(torch.empty(n_hist, history_steps))
                    self.seasonal_weight = nn.Parameter(torch.empty(n_hist, history_steps))
                    self.trend_bias = nn.Parameter(torch.zeros(n_hist))
                    self.seasonal_bias = nn.Parameter(torch.zeros(n_hist))
                ...
                self.control_head = nn.Sequential(
                    nn.LayerNorm(n_future),
                    nn.Linear(n_future, hidden),
                    nn.GELU(),
                    nn.Dropout(dropout),
                )
                self.out = nn.Linear(hidden * 2, n_targets)

            def moving_average(self, history):
                x = history.transpose(1, 2)
                trend = self.avg_pool(x)
                if trend.shape[-1] != history_steps:
                    trend = trend[..., :history_steps]
                return trend.transpose(1, 2)

            def forward(self, history, future):
                trend = self.moving_average(history)
                seasonal = history - trend
                trend_t = trend.transpose(1, 2)
                seasonal_t = seasonal.transpose(1, 2)
                if self.individual:
                    trend_repr = (trend_t * self.trend_weight.unsqueeze(0)).sum(dim=-1) + self.trend_bias
                    seasonal_repr = (seasonal_t * self.seasonal_weight.unsqueeze(0)).sum(dim=-1) + self.seasonal_bias
                else:
                    trend_repr = self.trend(trend_t).squeeze(-1)
                    seasonal_repr = self.seasonal(seasonal_t).squeeze(-1)
                hist = self.history_head(trend_repr + seasonal_repr)
                ctrl = self.control_head(future)
                return self.out(torch_mod.cat([hist, ctrl], dim=-1))
...
def per_batch_metric_rows(
    *,
    ...
    batch_ids = np.asarray(pred["batch_id"], dtype=np.int64)
    for batch_id in sorted(np.unique(batch_ids).tolist()):
        mask = batch_ids == batch_id
        m = regression_metrics(pred["true_raw"][mask], pred["pred_raw"][mask], target_scales)
        rows.append(
            {
                **base_fields,
                "batch_id": int(batch_id),
                "target": "__target_average__",
                "nrmse": float(m["mean_nrmse"]),
                "mae": float(m["mean_mae"]),
                "r2": float(m["mean_r2"]) if np.isfinite(m["mean_r2"]) else "",
...
def get_shard() -> tuple[int, int]:
    ...
    shard_index = int(os.environ.get("PHASE31_SHARD_INDEX", "0"))
    if shard_count < 1:
...
def main() -> None:
        ...
        for horizon in cfg["horizons"]:
            for control_mode in cfg["control_modes"]:
                ...
                for model_name in cfg["models"]:
                    ...
                    write_rows(metrics_dir / f"per_batch_metrics_partial{shard_suffix}.csv", per_batch_rows)
    ...
    write_rows(metrics_dir / f"per_batch_metrics{shard_suffix}.csv", per_batch_rows)
