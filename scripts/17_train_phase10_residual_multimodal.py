...
def import_torch() -> tuple[Any, Any, Any]:
    try:
        ...
        raise SystemExit("PyTorch is required for residual TCN training.") from exc
...
def build_model_class(torch: Any, nn: Any) -> Any:
    ...
    class ResidualTargetAdaptiveTCN(nn.Module):
        def __init__(
            ...
            if self.residual_model == "residual_conservative_target_gate_tcn":
                final_raman = self.raman_residual_head[-1]
                final_gate = self.target_gate[-2]
                if hasattr(final_raman, "weight"):
                    nn.init.zeros_(final_raman.weight)
                    nn.init.zeros_(final_raman.bias)
        ...
        def forward(
            ...
            if self.residual_model == "residual_process_tcn":
                ...
                if self.residual_model == "residual_naive_raman_tcn":
                ...
                elif self.residual_model in {"residual_target_gate_tcn", "residual_conservative_target_gate_tcn"}:
                    process_residual = self.process_head(base)
                    raman_residual = self.raman_residual_head(fused_input)
                    ...
                    aux["raman_delta"] = gate * raman_residual
                else:
    ...
    *,
    ...
    for epoch in range(1, int(cfg["training"]["epochs"]) + 1):
        ...
        for batch_idx, batch in enumerate(train_loader, start=1):
            ...
            pred, aux = model(history, future_exogenous, raman, baseline_scaled, return_aux=True)
            mse_loss = loss_fn(pred, target)
            loss = mse_loss
            gate_l1_weight = float(cfg["training"].get("gate_l1_weight", 0.0))
            raman_delta_l2_weight = float(cfg["training"].get("raman_delta_l2_weight", 0.0))
            if residual_model == "residual_conservative_target_gate_tcn":
                if gate_l1_weight > 0.0 and "target_gate" in aux:
                    loss = loss + gate_l1_weight * aux["target_gate"].mean()
                if raman_delta_l2_weight > 0.0 and "raman_delta" in aux:
                    loss = loss + raman_delta_l2_weight * torch.mean(aux["raman_delta"] ** 2)
            if not bool(torch.isfinite(loss).detach().cpu()):
                raise FloatingPointError(f"{run_name}: epoch={epoch} batch={batch_idx} non-finite train loss")
            loss_value = float(mse_loss.detach().cpu())
